...
import { ShoppingCart, Heart, Search, User, Menu, X, ChevronLeft, Minus, Plus, Trash2, LogOut, Package, Edit, Save, Upload, CheckCircle, MapPin, Phone, Facebook, MessageCircle, Sparkles } from 'lucide-react';
...
const Header = ({ onNavigate, cartCount, onSearch, onToggleMenu, isLoggedIn, categories }) => {
...
const ProductCard = ({ product, onNavigate, onAddToCart, onAddToWishlist }) => (
  <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-gray-200 text-right">
    <div className="relative group overflow-hidden">
      ...
      <div className="absolute top-2 right-2 p-2 bg-white bg-opacity-75 rounded-full">
        <button onClick={() => onAddToWishlist(product.id)} className="text-gray-500 hover:text-red-500 transition-colors">
          <Heart size={24} />
        </button>
      </div>
    </div>
...
const ProductList = ({ products, onNavigate, onAddToCart, onAddToWishlist, categories, onFilter, selectedCategory, searchQuery }) => (
...
const ProductDetail = ({ product, onNavigate, onAddToCart, onAddToWishlist, onGetRecommendations, recommendedProducts, recommendationsLoading }) => {
  if (!product) {
    return (
...
const ProductManagement = ({ products, onNavigate, onEditProduct }) => (
    <div className="container mx-auto p-4 md:p-8 pt-24 md:pt-32 min-h-screen text-right">
        <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-6">إدارة المنتجات</h1>
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 overflow-x-auto">
            <table className="min-w-full text-right table-auto">
                <thead>
                    <tr className="bg-slate-100 text-slate-600 text-sm font-semibold uppercase tracking-wider rounded-lg">
                ...
                <tbody>
                    {products.map(product => (
                        <tr key={product.id} className="border-b border-gray-200 last:border-b-0 text-slate-700">
                            ...
                            <td className="py-3 px-4">
                                <button onClick={() => onEditProduct(product.id)} className="flex items-center gap-1 text-slate-700 hover:text-slate-900 transition-colors">
                                    <Edit size={16} />
                                    <span>تعديل</span>
                                </button>
                            </td>
...
const CategoryManagement = ({ categories, onNavigate, onEditCategory }) => (
    <div className="container mx-auto p-4 md:p-8 pt-24 md:pt-32 min-h-screen text-right">
        <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-6">إدارة الفئات</h1>
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 overflow-x-auto">
            <table className="min-w-full text-right table-auto">
                <thead>
                    <tr className="bg-slate-100 text-slate-600 text-sm font-semibold uppercase tracking-wider rounded-lg">
                ...
                <tbody>
                    {categories.map(category => (
                        <tr key={category.id} className="border-b border-gray-200 last:border-b-0 text-slate-700">
                            ...
                            <td className="py-3 px-4">
                                <button onClick={() => onEditCategory(category.id)} className="flex items-center gap-1 text-slate-700 hover:text-slate-900 transition-colors">
                                    <Edit size={16} />
                                    <span>تعديل</span>
                                </button>
                            </td>
...
export default function App() {
  ...
  switch (view) {
    ...
    case 'productManagement':
        content = isLoggedIn ? <ProductManagement products={productsData} onNavigate={navigate} onEditProduct={(id) => navigate('editProduct', { productId: id })} /> : <Login onLogin={setIsLoggedIn} onNavigate={navigate} />;
        break;
    case 'categoryManagement':
        ...
        }} /> : <Login onLogin={setIsLoggedIn} onNavigate={navigate} />;
        break;
    case 'editProduct':
  ...
  return (
    <div className="font-sans bg-slate-50 text-slate-800 min-h-screen flex flex-col">
      ...
      <Header onNavigate={navigate} cartCount={cart.length} onSearch={handleSearch} onToggleMenu={toggleMenu} isLoggedIn={isLoggedIn} categories={categoriesData} />
